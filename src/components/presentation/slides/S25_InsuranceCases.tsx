...
const ICONS = [FiZap, FiShield, FiClock, FiCheckCircle];

export function S25_InsuranceCases() {
  const { t } = useTranslation('presentation');
  const c = usePresentationColors();
  const cases = t('insuranceCases.cases', { returnObjects: true }) as Array<{
    company: string;
    segment: string;
    metric: string;
    metricLabel: string;
        ...
        <MotionBox initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          <VStack spacing={2} textAlign="center">
            <Text fontSize="xs" fontWeight="700" textTransform="uppercase" letterSpacing="widest" color="brand.400">
              {t('insuranceCases.badge')}
            </Text>
            <Heading fontSize={{ base: '2xl', md: '4xl', lg: '5xl' }} fontWeight="800" lineHeight="1.15">
              {t('insuranceCases.headline')}{' '}
              <Text as="span" color="brand.400">{t('insuranceCases.headlineHighlight')}</Text>
            </Heading>
            <Text fontSize={{ base: 'sm', md: 'md' }} color={c.textMuted} maxW="650px">
              {t('insuranceCases.subtitle')}
            </Text>
          </VStack>
        </MotionBox>
