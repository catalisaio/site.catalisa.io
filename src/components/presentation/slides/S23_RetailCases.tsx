...
import { FiShoppingCart, FiMessageCircle, FiUsers, FiClock } from 'react-icons/fi';
import { useTranslation } from 'react-i18next';
import { Slide } from '../Slide';
import { MotionBox } from '../../motion';
import { usePresentationColors } from '../PresentationThemeContext';

const ICONS = [FiShoppingCart, FiMessageCircle, FiUsers, FiClock];

export function S23_RetailCases() {
  const { t } = useTranslation('presentation');
  const c = usePresentationColors();
  const cases = t('retailCases.cases', { returnObjects: true }) as Array<{
    company: string;
    segment: string;
    metric: string;
    metricLabel: string;
        ...
        <MotionBox initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          <VStack spacing={2} textAlign="center">
            ...
            <Heading fontSize={{ base: '2xl', md: '4xl', lg: '5xl' }} fontWeight="800" lineHeight="1.15">
              {t('retailCases.headline')}{' '}
              <Text as="span" color="brand.400">{t('retailCases.headlineHighlight')}</Text>
            </Heading>
            <Text fontSize={{ base: 'sm', md: 'md' }} color={c.textMuted} maxW="650px">
              {t('retailCases.subtitle')}
            </Text>
          </VStack>
        </MotionBox>
